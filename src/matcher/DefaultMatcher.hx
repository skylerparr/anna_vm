...
class DefaultMatcher implements Matcher {
  ...
  public function match(left:MatchValue, right:Dynamic):MatchData {
    return tryMatch(left, right, new Map<String, Dynamic>());
  }

  private inline function tryMatch(left:MatchValue, right:Dynamic, matchedVars:Map<String, Dynamic>):MatchData {
    ...
    switch(left.type) {
      ...
      case MatchType.COMPLEX:
        matchData = matchComplex(left, right, matchedVars);
      case MatchType.HEAD_TAIL:
        matchData = matchHeadTail(left, right, matchedVars);
  ...
  private inline function matchHeadTail(left:MatchValue, right:Dynamic, matchedVars:Map<String, Dynamic>):MatchData {
    ...
    matchData = tryMatch(listLeft[0], listRight.pop(), matchedVars);
    if (matchData.matched) {
      left = listLeft[1];
      matchData = tryMatch(left.value, right, matchedVars);
    }
    return matchData;
  }

  private inline function matchComplex(left:MatchValue, right:Dynamic, matchedVars:Map<String, Dynamic>):MatchData {
    var matchData: MatchData = {matched: false, matchedVars: matchedVars};
    if (right.type == Types.TUPLE && left.value.type == Types.TUPLE) {
      matchData = matchTuple(left, right, matchedVars);
    }
    if (right.type == Types.LIST && left.value.type == Types.LIST) {
      matchData = matchList(left, right, matchedVars);
    }
    if (right.type == Types.MAP && left.value.type == Types.MAP) {
      matchData = matchMap(left, right, matchedVars);
    }
    return matchData;
  }

  private inline function matchTuple(left:MatchValue, right:Dynamic, matchedVars:Map<String, Dynamic>):MatchData {
    ...
    for (i in 0...arrayRight.length) {
      ...
      matchData = tryMatch(left, right, matchedVars);
      if (!matchData.matched) {
        updateUnmatched(matchData);
        break;
      }
  ...
  private inline function matchList(left:MatchValue, right:Dynamic, matchedVars:Map<String, Dynamic>):MatchData {
    var listRight:List<Dynamic> = cast(right.value, List<Dynamic>);
    ...
    for (item in listLeft) {
      ...
      matchData = tryMatch(item, rightItem, matchedVars);
      if (!matchData.matched) {
        updateUnmatched(matchData);
        break;
      }
  ...
  private inline function matchMap(left:MatchValue, right:Dynamic, matchedVars:Map<String, Dynamic>):MatchData {
    var matchData:MatchData = {matched: true, matchedVars: matchedVars};
    ...
    if(MapUtil.first(mapLeft) == null && MapUtil.first(mapRight) != null) {
      ...
      for (key in mapLeft.keys()) {
        ...
        if (mapRight.exists(keyValue)) {
          var matchData:MatchData = tryMatch(mapLeft.get(key), mapRight.get(keyValue), matchedVars);
          if (!matchData.matched) {
            updateUnmatched(matchData);
          }
        } else {
